...
3. Finally, we could also think about if/how the carbon footprint of launches offsets the benefits of a SDC. Now, I will treat this last item as speculative mostly because it is out of my wheelhouse.
 ...
 If achieving AGI truly requires gigawatt-scale data centers that cannot be sustainably scaled on Earth, then the upfront carbon cost of launches may become a secondary concern. In such scenarios, the choice might be between terrestrial nuclear power (with its regulatory challenges) or space-based systems—making the carbon footprint of deployment less relevant than the fundamental feasibility of either approach. So, my current take is that by examining the first two aspects, we can get a sense of how well the business case of this company holds up against technology which is less speculative than the third point.
